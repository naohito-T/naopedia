...
そしてコード内の変更ではなくなるためテストが不要となる(これは言い回しでどうにかなりそう)
...
ビジネスロジックをデータアクセスと切り離すというのは保存先がファイルだろうと、RDBだろうとドキュメントDBだろうと、ビジネスロジックはそれを気にかけないようにするため(要は保存をかける時にはバリデーションや整形など全てを気にしなくて良くする)
...
バリデーションはなるべく上の層でしてしまうのがいい。なるべく早めにバリデーションにかけてしまうことでサーバの負荷も下がる。
