...
Basic認証では、Authorizationヘッダーに、「Basic」の文字とその後に1つ空白を挟んで、ユーザー名とパスワードから生成した認証情報を指定します。
...
1. Expiresレスポンスヘッダー（HTTP1.0から存在するヘッダー）
2. Cache-Controlレスポンスヘッダー（HTTP1.1から定義されたヘッダー）
...
Cache-Controlはさまざまなキャッシュのコントロールを行うヘッダーで`max-age`以外にもさまざまな指定が可能。
