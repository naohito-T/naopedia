...
ただし**オブジェクトのプロパティの変更は追跡できる。**（jsのproxyを使用して）
...
.once修飾子をイベントに対して指定した時の動作は、イベントターゲットの要素にaddEventListener()メソッドでイベントリスナーを登録する際にaddEventListener()メソッドの第3引数のoptionsに `{ once: true }` を指定した時と同様の動作になります。
...
以下が一番基本のコンポーネント生成。いくら最新のvue, nuxtでのコンポーネント登録でもこれをやっていることに変わりはない。
...
**状態は一番上のコンポーネントだけで管理し、**これを変更するメソッドをPropsとして渡していく手法は一般的なものなので身につける。
